...
public class CommentTest {
    ...
    public void createComment() {
        ...
        Member author = Member.builder().email("test@gmail.com").password("DF#Q$FWAD").address(address).nickName("와바라바덥덥").build();
        Post post = Post.builder().member(author).description("테스트 설명글").title("테스트 제목").maximum(3).build();

        Member member = Member.builder().email("turieowp@gmail.com").password("asdadfafgadfWAD").address(address).nickName("띠용이").build();
    ...
    public void updateContent() {
        ...
        Member member = Member.builder().email("turieowp@gmail.com").password("asdadfafgadfWAD").address(address).nickName("쀼슝이").build();
        Comment comment = Comment.builder().member(member).post(post).content("혹시 휴지 사고 싶은데 휴지도 사나요??").build();
